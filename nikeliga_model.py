...
from config import (
    MARKETS, DECAY, CREDIBILITY_K, REFEREE_MIN, REFEREE_FULL,
...
def _referee_stats(subset: pd.DataFrame, referee: str, tot_col: str,
    ...
    totals = pd.to_numeric(ref_rows[tot_col], errors="coerce").dropna()
    ...
    hit_rates_home = _ou_hit_rates(
        pd.to_numeric(ref_rows[h_col], errors="coerce").dropna(), hr_lines
    ) if (h_col and hr_lines) else {}
    hit_rates_away = _ou_hit_rates(
        pd.to_numeric(ref_rows[a_col], errors="coerce").dropna(), hr_lines
...
def _build_1x2_entries(
    ...
    from config import X1X2_KEYS, X1X2_SHRINKS
    shrink = X1X2_SHRINKS[market]
    key = X1X2_KEYS[market]
    data = {
        **winner_probs(
            league_h + shrink * (lambda_home - league_h),
            league_a + shrink * (lambda_away - league_a),
            nb_r_home, nb_r_away,
        ),
        "home_record": _win_record(subset, home_team, "home", h_col, a_col),
        "away_record": _win_record(subset, away_team, "away", h_col, a_col),
    }
...
def _compute_market_lambdas(
    ...
    """Vráti (lambda_home, lambda_away, league_h, league_a, home_n, away_n, h2h_home, h2h_away)."""
    ...
    return lambda_home, lambda_away, league_h, league_a, home_s["n"], away_s["n"], h2h_home, h2h_away
...
def _compute_market_hit_rates(
    ...
    def _total(rows):
        return pd.to_numeric(rows[h_col], errors="coerce").add(
            pd.to_numeric(rows[a_col], errors="coerce"), fill_value=0
        ).dropna()
    ...
    result = {
        ...
        "hit_rates_home_stat":     _ou_hit_rates(pd.to_numeric(home_rows[h_col], errors="coerce").dropna(), HR_LINES),
        "hit_rates_away_stat":     _ou_hit_rates(pd.to_numeric(away_rows[a_col], errors="coerce").dropna(), HR_LINES),
        "hit_rates_home_opp_stat": _ou_hit_rates(pd.to_numeric(home_rows[a_col], errors="coerce").dropna(), HR_LINES),
        "hit_rates_away_opp_stat": _ou_hit_rates(pd.to_numeric(away_rows[h_col], errors="coerce").dropna(), HR_LINES),
...
def _predict_market(
    ...
    lambda_home, lambda_away, league_h, league_a, home_n, away_n, h2h_home, h2h_away = (
