...
def _parse_match_date(date_str: str):
    """Parsuje dátum v oboch formátoch: Niké liga (DD.MM.YYYY) aj Chance liga (DD/MM/YYYY)."""
    import re
...
def get_upcoming(data_dir_str: str, days_ahead: int = 3) -> list[dict]:
    ...
    for m in matches:
        ...
        # Preskočiť placeholder zápasy (napr. "víťaz 8/9", "vítěz 7/10") kde tímy nie sú ešte známe
...
def show_market(match_key: str, market: str, label: str, pred_data: dict,
    ...
    if x1x2_key and pred_data.get(x1x2_key):
        ...
        for col, (lbl, p, rec) in zip(cols, labels_1x2):
            with col:
...
with st.sidebar:
    ...
    st.caption(f"Dáta: {'nikeliga.sk' if 'nikeliga' in data_dir_str else 'chanceliga.cz'}")
    st.divider()
    ...
    if "nikeliga" in data_dir_str:
        ...
        from chanceliga_batch import discover_from_listing, fetch_matches, export_csv as chance_export_csv, _load_or_create_index
        if st.button("🔄 Aktualizovať zápasy", use_container_width=True):
            with st.spinner("Sťahujem zápasy zo chanceliga.cz..."):
            ...
            st.cache_data.clear()
            if errors:
                st.warning(f"Nové: {downloaded}, preskočené: {skipped}, chyby: {errors}")
            else:
                st.success(f"Hotovo — nové: {downloaded}, preskočené: {skipped}.")
            st.rerun()
